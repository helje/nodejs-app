...
pipeline {
        ...
        parameters{
            ...
            choice(name: 'ENVIRONMENT', choices: ['helje-ext-dev', 'helje-ext-stage'], description: 'The name of the environement where we want to deploy/build resources.')
	    ...
	    string(name: 'SOURCES_URL', defaultValue: 'https://github.com/helje/nodejs-app.git', description: 'Gitlab repository source of the application.')
                ...
                { 
                    script
                    {
                       sh """
			            ...
			            oc set image deployment/nodejs-image-demo nodejs-image-demo=image-registry.openshift-image-registry.svc:5000/helje-ext-dev/nodejs-image:${params.IMAGE_TAG}
		...
		stage("Create route to access the Application")
            	{
                 when {
                    expression { params.ENVIRONMENT == 'helje-ext-dev' && params.DEPLOYMENT_TYPE == 'DEPLOY' }
                 }
                steps
                { 
                    script
